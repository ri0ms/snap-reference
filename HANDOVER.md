...
| 진단 모드 | https://snap-reference-omega.vercel.app/?debug — 좌하단에 폭/높이 변경 횟수, 문서높이 변경, 다시그림 횟수, 위로 튄 횟수를 표시 |
...
- 강조색을 `--ink-60`(#6B6B68)보다 훨씬 어둡게 바꾸지 마세요. 탭에서 "색이 바뀐 것"으로 안 읽히고 "진해진 것"으로 보입니다. 짙은 남색·자주·올리브가 전부 여기서 실패했습니다.
- **`.tile` 에 `content-visibility:auto` 를 다시 넣지 마세요.** 넣었다가 뺐습니다 — 타일이 화면에 들어올 때마다 레이아웃·페인트를 몰아서 해 모바일 세로 스크롤이 끊겼습니다. 사진이 수백 장이 되면 재검토할 수 있지만, 수십 장 규모에서는 손해입니다.
- **`body` 에 `overflow-x:hidden` 을 쓰지 마세요.** 일부 휴대폰에서 스크롤 하드웨어 가속이 꺼집니다. 지금은 `html{overflow-x:clip}` 을 씁니다.
- 호버 전용 장식(`.tile .cap`, `filter` 전환)은 **터치 기기에서 만들지 않습니다**(`TOUCH` 플래그 / `@media (hover:hover)`). 사진마다 반투명 레이어가 하나씩 늘어납니다.
- **`resize` 에서 무조건 `renderGrid()` 를 부르지 마세요.** 휴대폰에서 아래로 스크롤하면 주소창이 숨으면서 **높이만** 바뀌는데, 그때마다 다시 그리면 사진이 처음부터 나타나고(깜빡임) 높이가 흔들려 스크롤이 맨 위로 되돌아갑니다. **무한 반복이 되어 아래로 내려갈 수 없습니다.** 반드시 `innerWidth` 가 실제로 달라졌을 때만 다시 그리세요(`lastW` 비교).
...
**사진이 많아질 때 먼저 손볼 곳** (2026-09-18 기준 23장, 여유 충분)

1. **`loadHome()` 이 보드의 사진 행을 전부 읽습니다** — 장수 세기와 대표 사진 고르기 때문입니다. 수백 장이 되면 홈 진입이 느려집니다. 카테고리별 `count` 쿼리 + `covers` 조회로 바꾸면 해결됩니다. **이게 첫 번째 천장입니다.**
2. **한 카테고리 300장 이상**이면 그리드 전체를 한 번에 그리는 방식이 무거워집니다. `loading="lazy"` 덕에 이미지 자체는 괜찮지만 DOM 요소가 장당 3개씩 늘어납니다. `더 보기` 방식(50장씩)으로 바꾸면 됩니다.
3. 용량은 장당 420KB~1MB → 1GB에 **1,000~2,400장**.

> 이전에 겪은 스크롤 깜빡임은 **사진 수와 무관**했습니다(주소창 resize 루프). 장수가 늘어 느려지는 것과는 원인이 다르니 혼동하지 마세요.
